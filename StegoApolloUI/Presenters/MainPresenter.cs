    ...
    public class MainPresenter
    {
        ...
        public MainPresenter(IMainView view, IStegoService stego)
        {
            ...
            _view.EmbedRequested += async (s, e) => {
                ...
                if (result.Success) { _view.IsProcessed = true; _view.ShowImage(result.Image); LogManager.Instance.LogSuccess("藏密程序成功!"); }
                else { _view.ShowError(result.ErrorMessage); LogManager.Instance.LogError("藏密程序發生問題，動作沒有成功!"); }
            };
